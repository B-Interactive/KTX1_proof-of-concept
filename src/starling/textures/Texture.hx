...
class Texture
{
    ...
    /** Helper: Returns true if ByteArray data contains KTX1 header at offset 0. */
    private static function isKtx1Data(data:ByteArray):Bool
    {
        if (data == null || data.length < 12) return false;
        var ktx1Sig = [0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A];
        for (i in 0...ktx1Sig.length)
        {
            if (data[i] != ktx1Sig[i]) return false;
        }
        return true;
    }
    ...
    public static function fromData(data:Dynamic, options:TextureOptions=null):Texture
    {
        ...
        {
            // KTX1 detection and handling
            if (isKtx1Data(data))
            {
                var ktx = new Ktx1Data(data);
                return fromKtx1Data(ktx,
                    options.scale,
                    options.mipMapping,
                    options.onReady
                );
    ...
    *  @param scale      the scale factor of the created texture. This affects the reported
    *                    width and height of the texture object.
    *  @param useMipMaps If the KTX1 data contains mipmaps, this parameter controls if they
    ...
    *  @param premultipliedAlpha  Indicates if the KTX1 data contains pixels in PMA format.
    */
    public static function fromKtx1Data(ktxData:Ktx1Data, scale:Float=1, useMipMaps:Bool=true,
        async:Texture->Void=null, premultipliedAlpha:Bool=false):Texture
    {
        ...
        if (async != null)
            ...
            nativeTexture.uploadCompressedTextureFromByteArray(ktxData.data, 0, false);

        concreteTexture.onRestore = function(textureRoot:ConcreteTexture):Void {
            if (Std.isOfType(textureRoot.base, openfl.display3D.textures.Texture)) {
                cast(textureRoot.base, openfl.display3D.textures.Texture).uploadCompressedTextureFromByteArray(ktxData.data, 0, false);
            } else {
                trace("Warning: restore logic called for non-2D Texture type: " + Type.getClass(textureRoot.base));
            }
        };

        if (async != null) {
            nativeTexture.addEventListener(openfl.events.Event.TEXTURE_READY, function(_) {
                async(concreteTexture);
            });
        }
